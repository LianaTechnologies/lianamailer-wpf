jQuery( document ).ready( function($) {
	...
	$lianaMailerCustomField.click(function(e){
	...
	$builder.on( 'input', '.wpforms-field-option-lianamailer input.label', function() {
		var value = $(this).val();
	 ...
	 $builder.on( 'wpformsSaved', function( e, data ) {
		...
		if($('#wpforms-panel-settings').is(':visible') && $('#wpforms-panel-settings .wpforms-panel-sidebar-section-lianamailer_wpforms').hasClass('active')) {
			console.log('We are in settings/lianamailer');
			redirect = true;
		}
		if($('#wpforms-panel-fields').is(':visible') && $('.wpforms-panel-content').find('.wpforms-field-lianamailer').length > 0) {
			console.log('We are in form builder AND lianamailer field found');
			redirect = true;
		}
